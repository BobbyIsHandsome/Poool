...
public class Ball implements Drawable, Movable, Subject {
    ...
    /*
    notify methods notifies all th observers that there is a change in state
     */
    @Override
    ...
    /*
    Return a CueStick class object
     */
    public CueStick getCueStickObject(){
        return this.cueObj;
    }
    ...
    public void setBallType(BallType type) {
        ...
        if (this.type == BallType.CUEBALL) {
            this.shape.setOnMousePressed(
                    (actionEvent) -> {
                        if (this.hasStopped()) {
                            //notify observers when the ball changes its state
     ...
     * Make the cue stick of fixed length
     */
    public void registerCueBallMouseAction () {
        this.shape.setOnMouseDragged(
                (actionEvent) -> {
                    if (this.hasStopped()) {
                        ...
                        double cueX = this.shape.getCenterX();
                        double cueY = this.shape.getCenterY();

                        this.cueStick.setStartX(cueX);
                        this.cueStick.setStartY(cueY);

                        double mouseX = actionEvent.getSceneX();
                        double mouseY = actionEvent.getSceneY();

                        Double distanceBetweenMouseAndCue = Math.sqrt(Math.pow((mouseX - cueX), 2)
                                + Math.pow((mouseY - cueY), 2));
                        Double ratio = cueLength / distanceBetweenMouseAndCue;

                        this.cueStick.setEndX(cueX + (ratio * (mouseX - cueX)));
                        this.cueStick.setEndY(cueY + (ratio * (mouseY - cueY)));
                    }
                }
        );
